...
export class SettingAddMovieComponent extends SettingMovieAbstraction {
  ...
  public onClickButtonMediaLibrary(): void {
    if (this.editMovie.mediaLibraryId && this.editMovie.mediaLibraryId.trim() !== '') {
      ...
      this.unsubscribeSearchMovie();
      this.subscriptionSearch = this.tmdbOperationService.fetchSearchMovieInfoByMediaLibrary(this.editMovie.mediaLibraryId, this.editMovie.id).pipe(take(1), finalize(() => {
